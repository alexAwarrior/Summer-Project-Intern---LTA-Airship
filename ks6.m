...
% But cross-check against the actual air volume needed for ONE drop:
%   V_air_needed = m_payload / rho_air_op = 2.0 / 1.145 = 1.746 m³
%   V_ballonet_target = 1.746 × 1.20 (20% safety margin) = 2.095 m³
%   As fraction of hull: 2.095 / 6.5 ≈ 32% — within acceptable range.
% We use the physics-derived target (1.2 × V_air_needed), NOT the 25% rule,
% because the mission constraint (must compensate exactly one 2 kg drop) is
% more specific than the generic industry fraction.
% The 25% rule is a lower bound; our calculation gives ~32% which is fine.
%
% NOTE ON VARIABLE ORDERING: m_payload must be declared here (ahead of
% Section 5) because the ballonet bladder mass depends on it and must be
% known before M_hw is summed. Section 5 re-documents it with full context.
m_payload = 2.000;   % [kg]  one 2 kg parcel — single delivery, no reload
n_deliver = 1;       % [-]   one Yo-Yo deployment per mission

V_air_needed      = m_payload / rho_air_op;  % [m³] air to compensate ONE 2 kg drop
V_ballonet_target = V_air_needed * 1.20;     % [m³] +20% safety margin

% Bladder (TPU-coated nylon, 0.035 kg/m²):
%   Bladder modelled as sphere for surface area estimate.
%   Using V_ballonet_target (physics-derived) for mass calculation.
r_bladder_prelim = (3*V_ballonet_target / (4*pi))^(1/3);
S_bladder_prelim = 4*pi*r_bladder_prelim^2;
m_bladder        = 0.035 * S_bladder_prelim;
...
% m_payload = 2.000 kg and n_deliver = 1 are declared early in Section 4
% (before ballonet sizing) because the bladder mass depends on m_payload.
% They are used here as-is. Mission context:
%   - ONE 2 kg parcel per mission, deployed via single Yo-Yo winch manoeuvre.
%   - No reload. Airship returns empty after delivery.
%   - v2 incorrectly used n=4 (4 × 2 kg drops), which had no basis in the
%     proposal and caused the impossible 8.38 m³ ballonet calculation.

fprintf('[5] Payload: %.3f kg  (×%d delivery — single Yo-Yo deployment)\n\n', m_payload, n_deliver);
...
%% =========================================================================
%  SECTION 8: SOLAR IRRADIANCE MODEL — EMPIRICAL ASYMMETRIC GAUSSIAN
%             Fitted to real measured data: profilesolar.com, Delhi, Summer
%
%  WHY THE COSINE MODEL (v3 previously) IS REPLACED:
%  ─ The astronomical cosine model is physically derived but uses clear-sky
%    assumptions and a theoretical G_clearsky = 900 W/m².
%  ─ Real Delhi summer irradiance has been measured and published by
%    profilesolar.com as hourly capacity factor (kWh/kW installed) by season.
%  ─ The summer curve shows:
%      • Solar noon at ~11:20 local time (not 12:00 — equation of time offset)
%      • Asymmetric shape: morning rise is steeper than afternoon fall
%        → afternoon: dust haze, convective cloud, thermal heating of atmosphere
%      • Peak capacity factor: ~0.73 (equivalent to 730 W/m² effective irradiance)
%      • Sunrise effective: ~05:30, sunset: ~19:00 (13.5h solar window)
%  ─ An asymmetric Gaussian is fitted to the digitised summer curve.
%    RMSE of fit = 0.033 (excellent for an analytical function).
%
%  MODEL DEFINITION:
%    CF(t) = A × exp(−0.5 × ((t − μ)/σ(t))²)
%    where σ(t) = σ_L  if t < μ  (morning)
%                 σ_R  if t ≥ μ  (afternoon)
%
%  FITTED PARAMETERS (from profilesolar.com Delhi Summer digitisation):
%    A    = 0.7766   — peak capacity factor (dimensionless, 0–1)
%    μ    = 11.37 h  — solar noon offset (Delhi summer, equation of time)
%    σ_L  = 2.429 h  — morning half-width (steeper rise)
%    σ_R  = 2.997 h  — afternoon half-width (shallower fall, haze/heat)
%
%  INTERPRETATION:
%    CF(t) is kWh output per kW of installed panel per hour.
%    It already encodes: panel temperature loss, real-sky cloud/dust,
%    soiling, and cosine losses for Delhi summer conditions.
%    P_panel(t) = CF(t) × G_STC × A_solar × eta_STC
%    where G_STC = 1000 W/m², eta_STC = 0.11 (Ascent HL-25 rated).
%    We apply eta_MPPT and eta_charge on top (these are not in the CF).
%    We do NOT apply a separate tilt_factor or thermal derating —
%    they are already embedded in the measured CF values.
%
%  COMPARISON WITH PREVIOUS MODELS:
%    v1/v2: sin(π(t−6)/12) × 1000 W/m² — symmetric, wrong window, 7639 Wh/m²/day
%    v3a:   cosine(θz) × 900 W/m²       — symmetric, correct window, 7752 Wh/m²/day
%    v3b:   Asym-Gaussian fit (THIS)     — measured shape, 5282 Wh/m²/day
%    The v3b value of 5282 Wh/m²/day matches published Delhi summer GHI data
%    from IMD and NREL NSRDB (Delhi summer: 5.0–5.5 kWh/m²/day) ✓
%    The sine and cosine models were both significantly OVER-estimating daily
%    insolation (7600–7700 vs real 5200–5500). This is now corrected.
% =========================================================================

% Asymmetric Gaussian fit parameters (from profilesolar.com Delhi summer)
CF_A    = 0.7766;    % [-]  peak capacity factor
CF_mu   = 11.3684;   % [h]  solar noon (local time, Delhi summer ~11:22)
CF_sigL = 2.4293;    % [h]  morning sigma (steeper — lower airmass in morning)
CF_sigR = 2.9973;    % [h]  afternoon sigma (shallower — haze, heat, dust)
G_STC   = 1000.0;    % [W/m²] STC reference irradiance
...
% Compute capacity factor at each minute
CF_t = zeros(1, N_t);
for i = 1:N_t
    t_h = t_vec(i);
    if t_h < CF_mu
...
CF_t = max(CF_t, 0);   % clip any floating-point negatives (none expected)

% Irradiance at panel surface [W/m²] — G_eff encodes all real-world losses
G_t = CF_t * G_STC;   % [W/m²]

% Solar panel area
A_solar = 8 * (25 / (G_STC * eta_solar_stc));  % [m²] 8× Ascent HL-25 = 1.818 m²
%   Derivation: each 25W module rated at G_STC=1000 W/m², eta_STC=0.11
%   Area = 25 / (1000 × 0.11) = 0.2273 m² per module

% Panel power output (CF already includes temperature + soiling + sky losses)
P_solar_panel  = CF_t .* G_STC .* A_solar .* eta_solar_stc;  % [W] at panel terminals

% Power stored in battery (MPPT tracking + charge efficiency only)
% Do NOT apply tilt_factor or T_derating — already in the measured CF
P_solar_stored = P_solar_panel .* eta_MPPT .* eta_charge;    % [W] into battery

% Integrate: total solar energy stored per day
E_solar_stored = trapz(t_vec, P_solar_stored);   % [Wh]

% Peak values
[P_solar_peak, idx_peak] = max(P_solar_stored);
t_solar_peak = t_vec(idx_peak);

% Effective solar window
t_sunrise = t_vec(find(CF_t > 0.01, 1, 'first'));
t_sunset  = t_vec(find(CF_t > 0.01, 1, 'last'));

fprintf('[8] Solar Array — Empirical Asymmetric Gaussian [C3 updated]\n');
fprintf('    Source: profilesolar.com Delhi Summer measured data\n');
fprintf('    8× Ascent Solar HL-25 HyperLight CIGS 25W\n');
fprintf('    Panel area A_solar (8×0.2273 m²)     : %.4f m²\n', A_solar);
fprintf('    Fit: A=%.4f  μ=%.4f h  σL=%.4f h  σR=%.4f h\n', CF_A, CF_mu, CF_sigL, CF_sigR);
fprintf('    Peak capacity factor                  : %.4f  (at %.2fh)\n', CF_A, CF_mu);
fprintf('    Effective solar window                : %.2fh – %.2fh  (%.1f h)\n', t_sunrise, t_sunset, t_sunset-t_sunrise);
fprintf('    Peak panel power output               : %.1f W\n', max(P_solar_panel));
fprintf('    Peak power stored in battery          : %.1f W\n', P_solar_peak);
fprintf('    Total solar energy stored/day         : %.1f Wh\n', E_solar_stored);
fprintf('    [Prev cosine model gave 1050 Wh — was over-estimating by 10%%]\n\n');
...
% PASS 2: update ballonet bladder mass now that V_He is known, re-solve
% The ballonet target is fixed by physics (must hold V_air_needed × 1.2),
% NOT scaled from hull volume. Hull volume is determined by the mass balance.
% We just update the bladder mass since V_He is now precisely known.
V_ballonet_actual = V_ballonet_target;  % fixed by mission physics, not hull fraction
...
% Ballonet volume (fixed by physics — compensates ONE 2 kg drop)
V_ballonet = V_ballonet_target;           % [m³]  = V_air_needed × 1.2
...
fprintf('    Ballonet target (V_air×1.2)      : %.4f m³  (= %.0f%% of V_He) [1 drop]\n', V_ballonet, V_ballonet/V_He*100);
...
%   06:00–14:00  (8h):  Transit cruise to delivery location
%   14:00–14:30 (0.5h): ONE delivery sequence:
%                         • 5 min winch descent + Yo-Yo release
%                         • 30s RL buoyancy shock arrest (vertical motor burst)
%                         • ~5 min ballonet fill (restores neutral buoyancy)
%                         • Remainder: hover / station-keep
%   14:30–16:30  (2h):  Return cruise
%   16:30–00:00 (7.5h): Night standby (first half)
%   Total cruise:  10h  | Total night: 12h
%
%  NOTE: The airship makes ONE delivery. It carries ONE 2 kg package
%  and deploys it via a single Yo-Yo manoeuvre. There is no reload.
%  The buoyancy shock event (ONE × 30s) and ballonet fill (ONE cycle)
%  occur exactly once per mission.
% =========================================================================

% Mission durations [hours]
t_cruise_h   = 10.0;              % transit (8h outbound + 2h return)
t_hover_h    = 0.5;               % approach + station-keep at delivery site (30 min)
t_shock_h    = 30.0 / 3600;       % ONE buoyancy shock burst = 30 seconds
t_winch_h    = 5.0 / 60;          % ONE winch lowering operation = 5 minutes
t_fan_h      = t_fill_min / 60;   % ONE ballonet fill after the drop
...
fprintf('    %-34s  %9.2f  %8.4f  %10.2f\n', 'Buoyancy shock burst ×1 (30s)', P_mode_shock, t_shock_h, E_shock);
fprintf('    %-34s  %9.2f  %8.3f  %10.1f\n', 'Winch operation ×1 (5 min)', P_mode_winch, t_winch_h, E_winch);
fprintf('    %-34s  %9.2f  %8.3f  %10.1f\n', 'Ballonet fan ×1 (one fill)', P_mode_fan, t_fan_h, E_fan);
...
fprintf('    → This occurs ONCE per mission — one drop, one shock event.\n\n');
...
fprintf('  Ballonet volume (1 drop + 20%%)  : %.4f m³  (= %.0f%% of V_He)\n', V_ballonet, V_ballonet/V_He*100);
...
for i = 1:length(eta_range)
    % Sweep STC efficiency: CF already encodes real-world losses;
    % varying eta_STC scales the panel output proportionally.
    P_sp = eta_range(i) * A_solar .* G_t .* eta_MPPT .* eta_charge;
...
% --- Plot 1: Solar irradiance model comparison (3 models) ---
ax1 = subplot(2,3,1);

% Model A: old symmetric sine (v1/v2) — for reference
...
% Model B: astronomical cosine (v3 previous) — for reference
phi_ref = 28.64*pi/180; delta_ref = 23.45*pi/180; G_clr_ref = 900;
G_cosine = zeros(1, N_t);
for i = 1:N_t
    h_r = (t_vec(i)-12)*15*pi/180;
    ctz = sin(phi_ref)*sin(delta_ref) + cos(phi_ref)*cos(delta_ref)*cos(h_r);
    G_cosine(i) = G_clr_ref * max(0, ctz);
end

% Model C: empirical asymmetric Gaussian (THIS VERSION — fitted to chart)
% G_t already computed above in Section 8

% Plot all three
plot(t_vec, G_sine_old, 'r--', 'LineWidth', 1.2, 'DisplayName', 'v1/v2: sine'); hold on;
plot(t_vec, G_cosine,   'b--', 'LineWidth', 1.2, 'DisplayName', 'v3a: cosine (astro)');
plot(t_vec, G_t,        'y-',  'LineWidth', 2.5, 'DisplayName', 'v3b: Asym-Gaussian (measured)', 'Color', [0.85 0.65 0.0]);
area(t_vec, G_t, 'FaceAlpha', 0.15, 'FaceColor', [1.0 0.85 0.2], 'EdgeColor', 'none');

% Mark digitised data points (rescaled to W/m²)
...
scatter(t_pts, CF_pts*1000, 20, [0.6 0.4 0], 'filled', 'DisplayName', 'profilesolar.com data');

xline(t_sunrise, 'k:', 'LineWidth', 1.0, 'HandleVisibility', 'off');
xline(t_sunset,  'k:', 'LineWidth', 1.0, 'HandleVisibility', 'off');
ylabel('Effective irradiance  [W/m²]'); xlabel('Hour of day');
title({'Solar Model: Empirical Fit', 'profilesolar.com Delhi Summer'});
legend('Location', 'n', 'FontSize', 7); grid on;
xlim([4 21]); xticks(4:2:21); ylim([0 1100]);
text(4.3, 950, sprintf('Sine: %.0f Wh/m²/d', trapz(t_vec,G_sine_old)), 'FontSize',7,'Color',[0.8 0 0]);
text(4.3, 870, sprintf('Cosine: %.0f Wh/m²/d', trapz(t_vec,G_cosine)), 'FontSize',7,'Color',[0 0 0.8]);
text(4.3, 790, sprintf('Fit: %.0f Wh/m²/d ✓', trapz(t_vec,G_t)), 'FontSize',7,'Color',[0.6 0.4 0],'FontWeight','bold');
text(4.3, 680, '(IMD/NREL: 5000-5500)', 'FontSize',6,'Color',[0.4 0.4 0.4]);
...
xline(eta_solar_stc*100, 'r--', sprintf('η_{STC}=%.1f%% (design)', eta_solar_stc*100), ...
